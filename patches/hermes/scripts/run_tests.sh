...
# 2) If a venv was found, get its python and verify pytest is available
if [ -n "$VENV" ]; then
  PYTHON_CANDIDATE="$VENV/$VENV_BIN/python"
  # Windows venvs often leave .exe extension
  if [ ! -x "$PYTHON_CANDIDATE" ] && [ -f "${PYTHON_CANDIDATE}.exe" ]; then
    PYTHON_CANDIDATE="${PYTHON_CANDIDATE}.exe"
  fi
  if "$PYTHON_CANDIDATE" -c 'import pytest' 2>/dev/null; then
    PYTHON="$PYTHON_CANDIDATE"
  fi
fi

# 3) Fall back to HERMES_PYTHON (Nix dev shell / portable python)
if [ -z "$PYTHON" ] && [ -n "${HERMES_PYTHON:-}" ] && [ -x "$HERMES_PYTHON" ] \
    && "$HERMES_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: HERMES_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `hermes` binary rather
  # than the devShell hook.
  PYTHON="$HERMES_PYTHON"
  echo "▶ using HERMES_PYTHON: $PYTHON"
fi
