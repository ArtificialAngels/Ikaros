    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if stream:
            ...
            def _on_reasoning(event_type, tool_name=None, preview=None, args=None, **kwargs):
                """Surface the model's true reasoning (thinking) to the
                OpenAI-wire SSE consumer.

                The agent emits ``("reasoning.available", "_thinking", <text>, None)``
                (see agent/conversation_loop.py).  The chat-completions path
                previously omitted ``tool_progress_callback`` entirely (it was
                intentionally left unwired to avoid duplicating the
                tool.started/completed emits owned by ``tool_start_callback``/
                ``tool_complete_callback``), which silently dropped every
                reasoning event before it could reach the wire.  We capture
                only ``reasoning.available`` here and forward it as a distinct
                ``event: hermes.reasoning`` SSE frame so downstream chat UIs
                (e.g. Ikaros conversation-tree) can render a thinking block.
                Tool lifecycle events are deliberately ignored — they remain
                owned by the structured callbacks above.
                """
        ...
        try:
            ...
            async def _emit(item):
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    ...
                    # Model's true reasoning (thinking) stream — forwarded as a
                    # named SSE event so non-OpenAI clients can render it.
