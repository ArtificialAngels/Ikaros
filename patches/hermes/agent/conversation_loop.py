        ...
        try:
            ...
            if (assistant_message.content and agent.tool_progress_callback):
                ...
                if first_line and getattr(agent, '_delegate_depth', 0) > 0:
                    ...
                    # Surface the model's TRUE reasoning (thinking) only when it
                    # actually produced a distinct reasoning field.  deepseek-v4-*
                    # and other reasoning models populate assistant_message.reasoning
                    # with the chain-of-thought (also copied to reasoning_content
                    # for the API at api_msg sync time).  When the model did NOT
                    # reason (or the provider does not expose a reasoning field),
                    # assistant_message.reasoning is empty — in that case we must
                    # NOT fall back to content, otherwise the answer text would be
                    # echoed into the thinking block and shown twice (once as
                    # thinking, once as content).  So emit only on a truthy reason.
                    _reason_text = getattr(assistant_message, "reasoning", None)
                    if _reason_text and _reason_text.strip():
                        try:
                            agent.tool_progress_callback("reasoning.available", "_thinking", _reason_text[:8000], None)
                        except Exception:
                            pass
