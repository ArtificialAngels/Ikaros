...
def _inject_ikaros_root_paths() -> None:
    """Ikaros patch (2026-08-02): derive IKAROS_* from the HERMES_HOME layout.

    Layout guard: only applies when HERMES_HOME points at
    ``<project_root>/data/hermes-agent`` and ``<project_root>/runtime`` exists.
    Derived values are written with ``os.environ[k] = v`` (overwrite) so a
    stale drive letter baked into ``.env`` cannot shadow the dynamic
    derivation, and MCP ``${IKAROS_*}`` placeholders keep resolving after the
    project moves to another drive without any external env injection.
    """
    try:
        from pathlib import Path
    ...
    if home.name != "hermes-agent" or home.parent.name != "data":
        return  # 非 Ikaros 标准结构 (<root>/data/hermes-agent)，跳过
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    try:
        ...
        try:
            ...
            pass
        # Ikaros patch: 从 HERMES_HOME 结构自推导 IKAROS_*（换盘符自动跟随，
        # 不依赖启动入口注入，也不被 .env 旧盘符覆盖）
        _inject_ikaros_root_paths()
