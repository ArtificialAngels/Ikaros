...
def load_paths_json(root: Path) -> dict:
    ...
    json_path = root / "Ikaros-environment" / "ikaros-paths.json"
...
def build_checks(root: Path, paths_cfg: dict) -> list[PathCheck]:
    ...
    checks.append(PathCheck(
        "Ikaros-memory 模块",
        str(root / "Ikaros-memory"),
        critical=True, must_be_dir=True
    ))
    checks.append(PathCheck(
        "记忆数据库",
        str(root / "Ikaros-memory" / "data" / "v3.db"),
    ...
    checks.append(PathCheck(
        "环境配置 (bat)",
        str(root / "Ikaros-environment" / "ikaros-env.bat"),
        critical=True, must_be_file=True
    ))
    checks.append(PathCheck(
        "环境配置 (ps1)",
        str(root / "Ikaros-environment" / "ikaros-env.ps1"),
        critical=True, must_be_file=True
    ))
    checks.append(PathCheck(
        "路径配置 (JSON)",
        str(root / "Ikaros-environment" / "ikaros-paths.json"),
        critical=True, must_be_file=True
    ))
...
def print_report(results: list[CheckResult]) -> tuple[int, int]:
    ...
    categories = [
        ...
        ("目录结构", ["目录"]),
        ("Hermes 组件", ["Hermes"]),
        ("Ikaros-memory", ["memory", "Embedding", "LLM 模型", "记忆"]),
        ("Ikaros-environment", ["环境配置", "路径配置"]),
    ]
