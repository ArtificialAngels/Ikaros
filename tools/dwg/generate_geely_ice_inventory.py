...
ROOT = Path(r"E:\KPSNC模具资料\GEELY_BHE20_ICE\2026-04-08_25-8715_Geely BHE20-ICE\1.2D3DMOLd\2D")
OUTPUT = ROOT / "模具文件清单.xlsx"

# === 解析规则 ===
PROJECT_PREFIXES = [r"25-8715"]
PREFIX_PATTERN = re.compile(r"^(" + "|".join(PROJECT_PREFIXES) + r")[\s_\-]*", re.IGNORECASE)

# 单个编号或范围编号：可在 _ 或字符串开头/单词边界
# 允许范围编号后接 _ 其他内容（如 6517~6518_定模螺旋冷却柱）
...
def parse_filename(stem: str) -> dict:
    ...
    if m:
        ...
        # RANGE 在开头：取匹配之后；否则取匹配之前
        if m.start() == 0:
            desc_part = s[m.end():].lstrip("_")
        else:
            desc_part = s[:m.start()].rstrip("_")
    else:
        m2 = SINGLE_NUM.search(s)
        if m2:
            range_start = range_end = m2.group(1)
            # 判断是开头/中间还是末尾匹配：
            #   末尾匹配 (m2.end() == len(s))：取匹配之前
            #   中间匹配 (m2.end() < len(s))：取匹配之后
            if m2.end() == len(s):
                desc_part = s[:m2.start()].rstrip("_")
            else:
                desc_part = s[m2.end():].lstrip("_")
        else:
            ...
            if lead:
                ...
                if m3:
                    ...
                    desc_part = s[:m3.start()] + s[m3.end():]
                else:
                    m4 = INNER_SINGLE.search(s)
                    if m4:
                        range_start = range_end = m4.group(1)
                        desc_part = s[:m4.start()] + s[m4.end():]
                    else:
                        desc_part = s
...
def main():
    ...
    total = 0
    skipped = 0
    SKIP_EXTS = {"log"}  # 不计入清单的扩展名
    for sd in subdirs:
        files = sorted([p for p in sd.iterdir() if p.is_file()])
        print(f"  📁 {sd.name}: {len(files)} 个文件")
        for fp in files:
            # 跳过系统文件
            if fp.name in {"Thumbs.db", ".DS_Store"} or fp.name.startswith("~$"):
                skipped += 1
                continue
            # 跳过纯日志等
            if fp.suffix.lower().lstrip(".") in SKIP_EXTS:
                skipped += 1
                continue
            total += 1
            stem = fp.stem
