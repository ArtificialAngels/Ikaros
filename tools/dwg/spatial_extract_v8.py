...
方法（按用户指导）：
1. 找"件号"文本位置 (X0, Y0)
2. 向上 (+Y) 找第一根横线（平行于 X 轴）→ 得到 4 个角点
3. 向左 (-X) 找第一根竖线（平行于 Y 轴）
4. 形成表格边界矩形 (X1,Y1) (X2,Y2) (X2,YL2) (XL2,YL2)
5. 边界内：标题行（件号所在 Y 附近）→ 列 X 定义
6. 边界内：数据行（Y < 标题 Y）→ 按 X 归类
"""
import ezdxf
    ...
    if e.dxftype() == 'LINE':
        try:
            ...
            lines.append({
                ...
                'is_h': abs(x1 - x2) > abs(y1 - y2),  # 水平线
                'is_v': abs(x1 - x2) < abs(y1 - y2),  # 垂直线
...
v_lines = [l for l in lines if l['is_v']]
...
for j in jianhao_list:
    print(f"  ({j['x']:.1f}, {j['y']:.1f})")

# ===== 对每个件号，找表格边界 =====
def find_table_boundary(x0, y0, h_lines, v_lines):
    """
    从件号位置 (x0, y0) 找：
    1) 向上 +Y 找第一根横线：过 (x0, ?) 最近的水平线
    2) 向左 -X 找第一根竖线：在该横线范围内，过 (?, y0) 最近的垂直线
    返回 4 个角点 (x_left, y_top), (x_right, y_top), (x_right, y_bottom), (x_left, y_bottom)
    """
    # 1) 找"上方最近"的水平线（Y > y0，且包含 x0）
    # 排序：Y 大的在上
    above_h = [l for l in h_lines if l['y1'] >= y0 - 1 and l['x1'] - 50 <= x0 <= l['x2'] + 50]
    if not above_h:
        return None
    # 找 Y 最大的
    above_h.sort(key=lambda l: -l['y1'])
    top_line = above_h[0]
    y_top = top_line['y1']
    x_left_h = top_line['x1']
    x_right_h = top_line['x2']

    # 2) 在该横线 Y 附近，向左找最近的垂直线
    # 范围：Y 跨越 y0，X <= x0
    left_v = [l for l in v_lines
              if l['x1'] <= x0 + 1
              and (l['y1'] - 5 <= y0 <= l['y2'] + 5 or l['y1'] - 5 <= y_top <= l['y2'] + 5)]
    if not left_v:
        return None
    # 找 X 最大的（最靠近 x0）
    left_v.sort(key=lambda l: -l['x1'])
    left_v_line = left_v[0]
    x_left = left_v_line['x1']

    # 3) 向下找：表格底部
    # 同样在该横线 Y 附近，向下找最远的水平线
    # 找所有 Y < y_top 且 Y > 某下界（最多 -500mm）的水平线
    below_h = [l for l in h_lines if l['y1'] < y_top - 1 and l['y1'] > y_top - 500
               and l['x1'] - 50 <= x0 <= l['x2'] + 50]
    if below_h:
        # Y 最大的（即最近的下方）
        below_h.sort(key=lambda l: -l['y1'])
        y_bottom = below_h[0]['y1']
    else:
        # 没找到则取 -200mm
        y_bottom = y_top - 200

    # 4) 右侧边界：X 方向找最右的竖线
    # 在 y_top ~ y_bottom 范围内
    right_v = [l for l in v_lines
               if l['x1'] >= x0 - 1
               and (l['y1'] - 5 <= y_bottom <= l['y2'] + 5 or l['y1'] - 5 <= y_top <= l['y2'] + 5)]
    if right_v:
        right_v.sort(key=lambda l: l['x1'])
        x_right = right_v[-1]['x1']
    else:
        x_right = x0 + 500
...
for jh in jianhao_list:
    ...
    if not boundary:
        print(f"  ❌ 件号 ({x0:.1f}, {y0:.1f}): 未找到边界")
        continue

    print(f"  ✓ 件号 ({x0:.1f}, {y0:.1f}): 边界 X=[{boundary['x_left']:.0f}, {boundary['x_right']:.0f}], Y=[{boundary['y_top']:.0f}, {boundary['y_bottom']:.0f}]")

    # 找标题行文字：边界内、Y 接近 y0（容差 5mm）、任意图层
    header = [t for t in text_items
              if boundary['x_left'] <= t['x'] <= boundary['x_right']
              and abs(t['y'] - y0) < 5.0
              and t['text'] in COLUMN_NAMES]
    header.sort(key=lambda t: t['x'])

    if len(header) < len(COLUMN_NAMES):
        print(f"    ⚠️ 标题不完整: {len(header)}/{len(COLUMN_NAMES)}")
        continue
...
for i, t in enumerate(tables):
    ...
    print(f"  表 {i+1}: 锚点 ({t['anchor_x']:.0f}, {t['anchor_y']:.0f})  "
          f"边界 X=[{b['x_left']:.0f},{b['x_right']:.0f}] Y=[{b['y_top']:.0f},{b['y_bottom']:.0f}]  "
