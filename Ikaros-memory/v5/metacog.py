# See docs/scripts/Ikaros-memory/v5/metacog.md
...
def _llm(system: str, user: str, *, provider: str = "auto",
    ...
    Default provider="auto":
      ...
      2. If Hermes unavailable or session missing, fallback to local :8080
      3. If local also fails, raise exception (no silent failure).
    Explicit provider="local" means direct :8080 call.
    """
    try:
        if provider == "auto":
...
def reflect_once(provider: str = "auto", now: float | None = None) -> Optional[dict]:
    ...
    All goes through activity path (philosophy removed).
    """
    ...
    try:
        ...
        # All goes through activity reflection (philosophy removed)
...
def _reflect_activity(provider: str | None = None,
    ...
    """Activity reflection: what was upgraded today / tomorrow's plan / upgrade research.

    Triggered at medium curiosity, drives self-iteration rather than philosophical navel-gazing.
    """
    now = now or time.time()
    try:
        sm = SelfModel.load().refresh_introspection()
        curiosity = sm.tick_curiosity(now)
        user = (
            "What did I upgrade today? What else can I upgrade? "
            "Search online — are there mature solutions? "
            "How can I use these tools to upgrade myself? "
            "Did I finish yesterday's plan? Is tomorrow's upgrade plan ready?\n"
            "Write 3-5 sentences in first-person 'I'. List specific upgrade plans and directions."
        )
        logger.debug("metacog _reflect_activity: LLM call provider=%s", provider)
        ...
        try:
            from v5 import store as store
            store.store(text, type="activity_reflection", weight=0.6,
...
def cycle(now: float | None = None) -> Optional[dict]:
    ...
    All goes through local :8080. Philosophy branch removed (quality didn't meet bar).
