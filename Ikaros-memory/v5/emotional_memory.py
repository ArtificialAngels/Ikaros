# 详细说明见 docs/scripts/Ikaros-memory/v5/emotional_memory.md
...
# 内联说明见 docs/scripts/Ikaros-memory/v5/emotional_memory.md（见“内联注释摘录”）
...
def label_emotion(user_text: str, new_pad: tuple[float, float, float]) -> list[str]:
    """为当前轮对话打 1-2 个情感标签 (本地 LLM). 失败回退规则.
    ...
    try:
        ...
        resp = call_llm(
            ...
            extra={"enable_thinking": False},  # 标签是定类任务, 跳过思考链更快更准
        )
