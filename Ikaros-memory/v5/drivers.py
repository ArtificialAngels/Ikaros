...
# ═══════════════════════════════════════════════════════════════
# 4) 三合一 Orchestrator — 替换 think.py 的核心循环
# ═══════════════════════════════════════════════════════════════
...
class ThinkingOrchestrator:
    ...
    用法:
        ...
        state = orch.tick()   # 一步步驱动
    """
    chaos: LorenzPAD = field(default_factory=LorenzPAD)
    eca: ECAGrid = field(default_factory=ECAGrid)
    ais: AISDetectorSet = field(default_factory=AISDetectorSet)
    last_pad: tuple[float, float, float] = (0.2, 0.0, -0.1)  # Ikaros baseline

    def tick(self, affect_pad: tuple[float, float, float] | None = None,
             memories: list[tuple[int, tuple[float, float, float]]] | None = None,
             blend: float = 0.3) -> dict:
        """
        一次思考 tick.

        Args:
            affect_pad: 当前 AffectState 的 (p, a, d). None = 只用 Lorenz.
            memories: V4 记忆 PAD 批 [(id, (p,a,d)), ...]. None = 跳过 AIS.
            blend: Lorenz 与 affect_pad 混合权重.

        Returns:
            {
                "pad": (p, a, d),           # 最终 PAD
                "topic": str,                # ECA 主导思考主题
                "activity": float,            # 思维活跃度 0..1
                "novelty": [(score, id), ...] # 记忆新颖性排序, 或 []
            }
        """
        # 1) Chaos PAD
        if affect_pad is not None:
            final_pad = self.chaos.blend(affect_pad, blend_factor=blend)
        else:
            final_pad = self.chaos.tick()
        self.last_pad = final_pad

        # 2) ECA 主题
        topic = self.eca.tick()
        activity = self.eca.activity_ratio()

        # 3) AIS 新颖性
        novelty_list: list[tuple[float, int]] = []
        if memories:
            novelty_list = self.ais.tick(memories)
...
if __name__ == "__main__":
    ...
    for _ in range(20):
        p, a, d = l.tick()
        print(f"  step {l.step:3d}: P={p:+.3f} A={a:+.3f} D={d:+.3f}")

    # Test 2: ECAGrid
    print("\n=== ECAGrid (Rule 110, 41 cells) ===")
    e = ECAGrid()
    activites = []
    for _ in range(50):
        topic = e.tick()
        activites.append(e.activity_ratio())
    topics_seen = set()
    e2 = ECAGrid()
    for _ in range(100):
        topics_seen.add(e2.tick())
    print(f"  Topics seen in 100 ticks: {', '.join(sorted(topics_seen))}")
    print(f"  Avg activity: {sum(activites)/len(activites):.3f}")

    # Test 3: AISDetectorSet
    print("\n=== AIS DetectorSet ===")
    ais = AISDetectorSet()
    # 模拟 batch: 10 条记忆
    test_mems = [
        (1, (0.2, 0.0, -0.1)),   # baseline — 应该很熟悉
        (2, (0.8, 0.7, 0.5)),    # 高愉悦 + 高唤醒 — 新颖
        (3, (-0.6, -0.5, -0.4)),  # 低落 — 新颖
        (4, (0.2, 0.1, -0.1)),   # baseline 附近 — 熟悉
        (5, (0.0, 0.0, 0.0)),    # 中性 — 可能熟悉
    ]
    # 先跑几次, 让检测器"学习" baseline
    for _ in range(5):
        ais.tick([(1, (0.2, 0.0, -0.1))])
    results = ais.tick(test_mems)
    print("  Novelty ranking:")
    for nov, mid in results:
        print(f"    mem #{mid}: novelty={nov:.3f}")

    # Test 4: Orchestrator
    print("\n=== ThinkingOrchestrator (10 ticks) ===")
    orch = ThinkingOrchestrator()
    for i in range(10):
        state = orch.tick(
            affect_pad=(0.2, 0.0, -0.1),
            memories=test_mems,
            blend=0.3,
        )
        print(f"  tick {i}: PAD=({state['pad'][0]:+.2f},{state['pad'][1]:+.2f},"
              f"{state['pad'][2]:+.2f}) topic='{state['topic']}' "
              f"activity={state['activity']:.2f} "
              f"novelty_top={state['novelty'][0] if state['novelty'] else '—'}")
