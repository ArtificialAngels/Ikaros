@echo off
REM ============================================================
REM  🪶 Icarus Desktop Pet — Hermes service launcher
REM
...
if errorlevel 1 (
    echo [icarus] FATAL: could not resolve HERMES_ROOT.
    exit /b 1
)
...
echo [icarus] Unknown mode: %MODE%
echo [icarus] Usage: bin\hermes-pet.bat [start^|stop^|status^|--autostart]
exit /b 1

:start
echo.
echo ============================================================
echo   🪶 Icarus Desktop Pet — Starting
echo ============================================================
set "PET_DIR=%HERMES_ROOT%\bin\icarus-desktop-pet"
set "LAUNCHER=%PET_DIR%\detached.py"
set "LOG=%HERMES_ROOT%\data\logs\icarus-pet.log"

if not exist "%LAUNCHER%" (
    echo [icarus] ERROR: %LAUNCHER% not found
    exit /b 1
)
...
start "IcarusPet" /B "%HERMES_PYTHON%" "%LAUNCHER%"

REM Wait a moment, then check
timeout /t 3 /nobreak >nul

REM Check if pet started (look for python with icarus-desktop-pet in cmdline)
wmic process where "name='python.exe' and commandline like '%%icarus-desktop-pet%%'" get processid /format:value 2>nul | find "ProcessId=" >nul
if errorlevel 1 (
    echo [icarus] WARNING: pet may not have started. Check log: %LOG%
) else (
    echo [icarus] ✓ Pet started.
)
echo.
exit /b 0
...
:stop
echo.
echo [icarus] Stopping Desktop Pet...
REM Find and kill python processes with icarus-desktop-pet in command line
set "KILLED=0"
for /f "tokens=2 delims==" %%P in ('wmic process where "name='python.exe' and commandline like '%%icarus-desktop-pet%%'" get processid /format:value 2^>nul') do (
    if not "%%P"=="" (
        echo   killing PID %%P...
...
taskkill /FI "WINDOWTITLE eq 💬*" /F >nul 2>&1
if "%KILLED%"=="1" (echo [icarus] ✓ Stopped.) else (echo [icarus] Pet was not running.)
echo.
exit /b 0
...
echo.
echo ============================================================
echo   🪶 Icarus Desktop Pet — HKCU Run Autostart
echo ============================================================
...
set "CMD=%HERMES_PYTHON% %HERMES_ROOT%\bin\icarus-desktop-pet\detached.py"
reg add "%REG_KEY%" /v "%VAL_NAME%" /t REG_SZ /d "%CMD%" /f >nul
if errorlevel 1 (
    echo [icarus] ERROR: Failed to write registry (need admin?)
    echo   Try: bin\hermes-pet.bat start
    exit /b 1
)
echo [icarus] ✓ Autostart registered.
echo   Command: %CMD%
