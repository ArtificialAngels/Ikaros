...
SWITCH_BAT = HERMES_ROOT / "bin" / "switch-model.bat"
...
def print_models(models: list[dict], current: Optional[str] = None) -> None:
    if not models:
        print(f"[!] no .gguf models in {MODELS_DIR}")
        print("    add via: hermes-models.py download <url>")
        return
    print()
    print(f"{'#':<3} {'NAME':<40} {'SIZE':>8} {'QUANT':<10} {'ARCH':<14} {'CTX':>8} {'TENSORS':>8}")
    print("-" * 96)
    for i, m in enumerate(models, 1):
        ...
        nt = m["n_tensors"] or 0
        print(f"{mark}{i:<3} {m['name']:<40} {m['size_gb']:>6}GB {m['quant'] or '?':<10} {arch:<14} {ctx:>8} {nt:>8}")


# ---------- switching ----------

def switch_model(model_name: str) -> int:
    if not SWITCH_BAT.exists():
        print(f"[FAIL] {SWITCH_BAT} not found")
        return 1
    print(f"[*] switching to: {model_name}")
    rc = subprocess.call([str(SWITCH_BAT), model_name], cwd=str(HERMES_ROOT))
    return rc
...
def main():
    p = argparse.ArgumentParser(prog="hermes-models", description="Hermes Agent multi-model manager")
    sub = p.add_subparsers(dest="cmd")

    sub.add_parser("list", help="list local GGUF models")
    sub.add_parser("gopeed", help="list active gopeed download tasks")

    sw = sub.add_parser("switch", help="switch active model")
    ...
    if args.cmd == "list":
        models = list_models()
        print_models(models, current_model_from_bat())
        return 0
    if args.cmd == "switch":
        models = list_models()
        match = None
