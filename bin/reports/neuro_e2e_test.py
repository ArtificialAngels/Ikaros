...
logger = logging.getLogger("icarus.e2e")

from bridge.signals import icarus, AI_NAME, HOST_NAME
...
async def main():
    logger.info("=== Icarus Neuro E2E ===")
    logger.info(f"AI_NAME: {AI_NAME}, HOST_NAME: {HOST_NAME}")

    # 假装系统就绪
    icarus.stt_ready = True
    icarus.tts_ready = True
    icarus.llm_ready = True

    # 启动 memory reflection
    mem = Memory(icarus, enabled=True)
    ...
    icarus.mark_new_message("user", "你好伊卡洛斯")

    # 等几秒,看 prompter 处理
    await asyncio.sleep(0.5)
    logger.info(f"history len: {len(icarus.history)}")

    # 等 PATIENCE 触发
    logger.info("等 4s 看 PATIENCE...")
    await asyncio.sleep(4.0)
    logger.info(f"history len after patience: {len(icarus.history)}")
    ...
    for i in range(25):
        icarus.mark_new_message("user", f"测试消息 {i}")
        icarus.mark_new_message("assistant", f"回复 {i}")
        await asyncio.sleep(0.01)
    logger.info(f"history len: {len(icarus.history)}, processed: {mem.processed_count}")
    ...
    prompt = build_system_prompt(icarus, mods, base_prompt=f"你是 {AI_NAME}")
