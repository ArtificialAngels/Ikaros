@echo off
REM See docs/scripts/bin/ikaros-start.md
...
echo   Memory:    :8587 embedding + :8080 local LLM
...
REM ---- Hermes Dashboard (MANUAL backup; rationale: docs/scripts/bin/ikaros-start.md) ----

REM ---- 4. Hermes Studio (hermes-web-ui, dev mode; rationale: docs/scripts/bin/ikaros-start.md) ----
...
REM Studio env set in PARENT (correct `set "VAR=val"`); see docs/scripts/bin/ikaros-start.md
set "HERMES_WEB_UI_HOME=%IKAROS_DATA%\hermes-studio"
set "IKAROS_NODEPATH_SAVE=%NODE_PATH%"
set "IKAROS_PATH_SAVE=%PATH%"
set "NODE_PATH="
set "PATH=%IKAROS_RUNTIME%\node;%PATH%"
rem Clear old log BEFORE launching new process (see docs/scripts/bin/ikaros-start.md)
if exist "%STUDIO_LOGS%\hermes-studio.log" del "%STUDIO_LOGS%\hermes-studio.log" 2>nul
wscript.exe "%IKAROS_BIN%\launch-hidden.vbs" "cmd /c cd /d ""%STUDIO%"" && npm run dev > ""%STUDIO_LOGS%\hermes-studio.log"" 2>&1"
set "NODE_PATH=%IKAROS_NODEPATH_SAVE%"
set "PATH=%IKAROS_PATH_SAVE%"
echo       Waiting for Hermes Studio on :8649...
set "W=0"
:studio_wait
...
ping -n 3 127.0.0.1 >nul
set /a W+=2
rem Print a dot every 10 seconds so the user sees progress
set /a mod=W%%10
if "%mod%"=="0" <nul set /p =.
