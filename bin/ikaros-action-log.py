#!/usr/bin/env python3
"""ikaros-action-log.py — 伊卡洛斯动作日志 (哥哥 7-3 19:20 拍板)

目的: 解决"hermes 网关死没死我不知道"的盲区. 5 维 + 状态 + 完成度.
...
状态字段:
  - intent:     一句话人话 (例: "kill llama-server pid 17572")
  - action:     动作类型
  - target:     target 描述
  - result:     "ok" | "fail" | "timeout" | "stuck"
  - duration_ms: 耗时
  - exit_code:   命令返回值
  - complete:    true/false
  - completion_pct: 0-100
  - started_at:  ISO 时间戳
  - ended_at:    ISO 时间戳 (完成时)

**关键**: 每条 action 必须有 START 和 END 两条 log. 没有 END = 卡死/崩溃.
用法:
  # CLI
