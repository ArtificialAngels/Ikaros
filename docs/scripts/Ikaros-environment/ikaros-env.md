...
### Hermes Agent 路径
- `IKAROS_HERMES_AGENT` = `%IKAROS_ROOT%\hermes-agent`
- `IKAROS_HERMES_HOME` = `%IKAROS_ROOT%\data\hermes-agent`

### Hermes Studio（hermes-web-ui 前端）
- `IKAROS_STUDIO` = `%IKAROS_ROOT%\hermes-studio`
- `IKAROS_STUDIO_LOGS` = `%IKAROS_LOGS%\hermes-studio`
- `IKAROS_STUDIO_DATA` = `%IKAROS_DATA%\hermes-studio`
- 同时注入 web UI 依赖的 Hermes Agent 服务变量：
