...
## 用途（原模块 docstring）
替换 2026-07-05 损坏的 V3 Memory Plugin AUTO-SYNCED 机制（外部同步，不修改 hermes-agent）。读取 V5 `self_model.json` + `axiom.md` + `v4.db` lessons，生成 `SOUL.md`，写入 `data/hermes-agent/SOUL.md`。
...
5. 经验教训（v4.db 高权重 rule/lesson/identity/axiom，前 5）
6. 能力清单（ikaros-identity/capabilities.md）

文件头标注 `<!-- AUTO-SYNCED ... DO NOT manually edit -->`。

## 路径自举
`IKAROS_ROOT` = env 或 `Path(__file__).resolve().parent.parent`；`IKAROS_MEMORY = IKAROS_ROOT/Ikaros-memory`；`HERMES_HOME` = env 或 `IKAROS_ROOT/data/hermes-agent`。`sys.path.insert(0, IKAROS_MEMORY)`。
