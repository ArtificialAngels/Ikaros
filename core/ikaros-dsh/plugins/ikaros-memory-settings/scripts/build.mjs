// ikaros-memory-settings 构建：Node 侧 tsc → dist/index.js；client 侧 esbuild CJS → dist/client.js
// 复用 ikaros-memory 的 tsc (同级 node_modules) + runtime/node 自带 esbuild。
// 同 ikaros-conversation-tree 的 build.mjs 设计。
...
const IKAROS = join(root, '..', '..', '..', '..')   // plugins/ikaros-memory-settings → ikaros-dsh → core → E:/Ikaros
...
const clientId = '@ikaros/dsh-ikaros-memory-settings'
