...
### 4.8 N.E.K.O (`neko.*`, NEW 2026-07-22) — 7 项

| Key | 作用 |
|---|---|
| `neko.root` | `%IKAROS_NEKO%` — N.E.K.O-main 根目录 |
| `neko.python` | `.venv\Scripts\python.exe` (Python 3.11) |
| `neko.server` | `app/main_server.py` — FastAPI 后端 |
| `neko.desktop` | `N.E.K.O.exe` — Electron 桌面壳 |
| `neko.static` | `static/` — 前端静态资源 |
| `neko.templates` | `templates/` — HTML 模板 |
| `neko.venv` | `.venv/` — Python 虚拟环境 |

### 4.9 Ports (`ports.*`) — 5+3 个常用端口
...
文件位置: `E:\Ikaros\core\env\PATH-LAYER.md`
